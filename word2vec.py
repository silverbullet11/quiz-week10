...
def maybe_download(filename, expected_bytes):
    ...
    if not os.path.exists(local_filename):
        print('Not find the cached file. Will download it from the internet...')
        local_filename, _ = urllib.request.urlretrieve(url+filename, local_filename)
    else:
        print('Found the cached file.')
...
# We pick a random validatoin set to sample nearest neighbors. Here we limit
...
with tf.Session(graph=graph) as session:
    ...
    for step in xrange(num_steps):
        batch_inputs, batch_labels = generate_batch(batch_size, num_skips=num_skips, skip_window=skip_window)
    ...
    try:
        ...
        print('Please install sklearn to show embeddings.')
        print(ex)