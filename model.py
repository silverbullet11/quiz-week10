...
class Model():
    ...
    def build(self, embedding_file=None):
        ...
        with tf.variable_scope('rnn'):
            ...
            # Your Code here
            ##################
            cell = tf.nn.rnn_cell.LSTMCell(self.dim_embedding , state_is_tuple=True)
            ...
            seq_outputs, self.outputs_state_tensor = tf.nn.dynamic_rnn(cell=cell, inputs=data, initial_state=self.state_tensor)


        # flatten it
        seq_output_final = tf.reshape(seq_outputs, [-1, self.dim_embedding])
